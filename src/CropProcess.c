...
void Processes(int y, int doy, int autoNitrogen, CropStruct *Crop, ResidueStruct *Residue, const WeatherStruct *Weather, SoilStruct *Soil, SoilCarbonStruct *SoilCarbon)
{
    ...
    double dailyGrowth;     /* apparent daily growth rate as it includes rizhodeposition */
    double N_AbgdConcReq, N_RootConcReq;    /* maximum concentration in daily growth - important! */
    double NxAbgd, NcAbgd, NnAbgd;          /* maximum, critical and minimum concentration of cumulative abgd  */
    double NaAbgd;                          /* actual nitrogen concentration in aboveground biomass */
    double NxRoot;                          /* maximum nitrogen concentration of cumulative root biomass */
    ...
    if (Crop->svRadiationInterception > 0)
    {
        ...
        /* compute reference N concentration of biomass and required concentration of new growth (Eulerian) */
...
{
    double ShootPartitioning;           /* fraction of daily growth allocated to aboveground biomass */
    ...
    double TUE;
    double RUE;
    double PRG;         /* potential growth based on radiation */ 
    double PTG;         /* potential growth based on transpiration */

    double RRD = 0.6;   /* root to rizhodeposition ratio of biomass allocated to roots */
    ...
    RUE = 0.01 * Crop->userRadiationUseEfficiency;      /* g/MJ solar radiation intercepted, 0.01 converts to Mg/ha */
    TUE = 0.01 * Crop->userTranspirationUseEfficiency * pow (daytimeVPD, -0.59);    /* 0.01 converts to Mg/ha */
    ShootPartitioning = ShootBiomassPartitioning (Stage, Crop->userShootPartitionInitial, Crop->userShootPartitionFinal);

    /* unstressed growth of aboveground biomass - might be used to calculate N demand */
    ...
    TranspirationGrowth = Crop->svTranspiration * TUE * (1. - pow (Crop->svN_StressFactor, 1.25));      /* this coefficient is higher to represent drop in WUE with N stress (radiation limits more) */
...
void CropNitrogenConcentration(double *N_AbgdConcReq, double *N_RootConcReq, double *NaAbgd, double *NxAbgd, double *NcAbgd, double *NnAbgd, double *NxRoot, const double Stage, const CropStruct *Crop)
{
    ...
    double BTNx, BTNc, BTNn;        /* threshold abgd biomass level after which N maximum / critical / minimum Conc dilution curve starts, Mg/ha */
    double NcEmergence, NnEmergence;        /* critical, minimum nitrogen concentration at crop emergence, maximum is passed */
    double Ax, Ac, An;                      /* biomass dilution curve constants */

    double Root_NxEmergence = 0.02;
    double R1 = 1.;                 /* root half constant for phenology-based dilution */
    double R2 = 0.33;               /* root power of equation for phenology-based dilution */
    ...
    /* compute current N concentration in aboveground biomass (this is reported back) */
    if (Crop->svN_Shoot > 0.)
    ...
    NcEmergence = Crop->userNMaxConcentration * 0.62;           /* Scaling from Nx to Ncritical */
    NnEmergence = Crop->userNMaxConcentration * 0.45;           /* Scaling from Nx to Nminimum */
    ...
    *NnAbgd = *NnAbgd < NnEmergence ? *NnAbgd : NnEmergence;
    ...
    /* based on biomass y = a*x^(-b); y'= -(a*b-a)/x^b where x = abgd biomass */
...
{
    double tempStress;
    double factor = 0.67;     /* weigthing of tempStress in determining today's plant stress */
...
void CropNitrogenUptake (double N_AbgdConcReq, double N_RootConcReq, double NaAbgd, double NxAbgd, double NxRoot, int autoNitrogen, CropStruct *Crop, SoilStruct *Soil)
{
    int i;
    double N_CropDemand;
    double N_ReqAbgdGrowth;     /* N mass required to satisfy abgd growth based on luxury consumption */
    double N_ReqRootGrowth;     /* N mass required to satisfy root growth based on luxury consumption */
    double N_ReqRhizodeposition;    /* N mass required to satisfy rizhodeposition */
    double N_AbgdConc, N_RootConc;  /* N concentration in biomass */

    double UptakeFactor[Soil->totalLayers]; /* factor used to increase soil supply in top layers and to decrease it when crop is maturing */
    double NO3Uptake[Soil->totalLayers];     /* array passed to Sub PotentialSoluteUptake() */
    double NH4Uptake[Soil->totalLayers];     /* array passed to Sub PotentialSoluteUptake() */
    double NO3supply, NH4Supply;            /* NO3 and NH4 cumulative supply through the rooting depth */
    double N_SoilSupply;                    /* NO3 and NH4 cumulative supply */
    double N_Uptake;                        /* NO3 and NH4 total N uptake */
    double N_FractionalSatisfiedDemand;
    double N_FractionalSatisfiedSupply;
    double Fraction_NO3, Fraction_NH4;      /* auxiliar variables to update NO3 and NH4 pools */
    double N_Surplus;                       /* variable used to remove excess N in biomass due to dilution */
    ...
    /* calculate N demand for today's growth (assume no deficit demand to be satisfied - testing) */
    ...
    PotentialSoluteUptakeOption2 (&NO3supply, NO3Uptake, 0, Soil->totalLayers, Soil->BD, Soil->layerThickness, Soil->waterUptake, Soil->NO3, Soil->waterContent);      /* Mg solute/ha */
    PotentialSoluteUptakeOption2 (&NH4Supply, NH4Uptake, 5.6, Soil->totalLayers, Soil->BD, Soil->layerThickness, Soil->waterUptake, Soil->NH4, Soil->waterContent);      /* Mg solute/ha */
...
{ 
    /* this sub considers the solute as "available" for uptake if in solution and in a layer with positive water uptake */
    /* 2014 02 20 a function was added to limit uptake if concentration is too low, this function only valid for NO3 and NH4 */
    ...
    double ppmSolute;               /* solute concentration in parts per million */
    double factorSoluteUptake;      /* factor the affects potential uptake based on ppm, current function valid for NO3 and NH4 */
    double temp1, temp2;
...
{
    double P1 = 0.4;            /* stage at which partitioning is half way between Po and Pf */
    double P2 = 4.;             /* curvature factor (not recommended to be available to user) */
...
void RadiationInterception (int y, int doy, CropStruct *Crop)
{
    ...
    double Reserves_Use_Allowance;      /* allows LAI expansion larger than daily growth * SLA */
    double Delta_Crop_Interception_Growth;  /* crop interception expansion based on daily growth and SLA */
    ...
    double a = 6;                    /* 5, 7 crop radiation interception parameter modified logistic curve */
    double b = 20;                     /*  crop radiation interception parameter modified logistic curve */
    double c = 15;                     /*  crop radiation interception parameter modified logistic curve */
    double d = 16;                         /* crop radiation interception parameter modified logistic curve [reduce the number increase senescence] */
    double e = 5;                          /* root parameter logistic curve */
    double f = 15;                         /* root parameter logistic curve */
    double rde = 0.3;                   /* m, root depth at emergence */
...
void ComputeColdDamage (int y, int doy, CropStruct *Crop,  const WeatherStruct *Weather, const SnowStruct *Snow, ResidueStruct *Residue)
{
    ...
    double Cold_Damage;     /* fraction of interception decreased by cold damage */
    double Crop_Tn;         /* minimum temperature for cold damage */
    double Crop_Tth;        /* threshold temperature for cold damage */
    double residueMass;
    double Residue_N_Mass;
    double Phenology_Delay_Factor;  /* the more advanced the cycle the lesser the effect on cold damage on delaying phenology */
    ...
    Cold_Damage = ColdDamage (Min_Temperature, Crop_Tn, Crop_Tth) * (1. - Snow->snowCover);      /* 20100516 snow cover protection added */
