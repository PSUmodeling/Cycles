...
void ComputeSoilCarbonBalance (SoilCarbonStruct *SoilCarbon, int y, ResidueStruct *Residue, SoilStruct *Soil, double *tillageFactor)
{
    ...
    for (i = 0; i < Soil->totalLayers; i++)
    {
        ...
        SoilCarbon->carbonRespired[i] = (1.0 - abgdHumificationFactor) * FRACTION_CARBON_PLANT * (xx1 + xx2 + xx3)
                            + (1.0 - rootHumificationFactor) * FRACTION_CARBON_PLANT * xx4
                            + (1.0 - rhizHumificationFactor) * FRACTION_CARBON_RIZHO * xx5
                            + (1.0 - manuHumificationFactor) * (xx6 + xx7)
