...
#else
    SoilCarbon->factorComposite =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->carbonRespired =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->rootBiomassInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->rhizBiomassInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->abgdBiomassInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->rootCarbonInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->rhizCarbonInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->manuCarbonInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->abgdCarbonInput =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->carbonMassInitial =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->carbonMassFinal =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualDecompositionFactor =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualSoilCarbonDecompositionRate =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualCarbonInputByLayer =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualHumifiedCarbonMass =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualRespiredCarbonMass =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualRespiredResidueCarbonMass =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualHumificationCoefficient =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualNmineralization =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualNImmobilization =
        (double *)calloc (totalLayers, sizeof (double));
    SoilCarbon->annualNNetMineralization =
        (double *)calloc (totalLayers, sizeof (double));
#endif
}
    ...
    int last_doy, soil_struct *Soil)
{
    ...
    for (i = 0; i < Soil->totalLayers; i++)
    {
#ifdef _PIHM_
        waterPotential =
            Psi ((Soil->waterContent[i] - Soil->smcmin) / (Soil->Porosity[i] -
                Soil->smcmin), Soil->alpha, Soil->beta) * GRAV;
#else
        waterPotential =
            SoilWaterPotential (Soil->Porosity[i], Soil->airEntryPotential[i],
            Soil->B_Value[i], Soil->waterContent[i]);
#endif
        ...
        SoilCarbon->factorComposite[i] =
            factorMoisture * factorAeration * factorTemperature;
        ...
        SoilCarbon->annualDecompositionFactor[i] +=
            SoilCarbon->factorComposite[i];
    ...
    residue_struct *Residue, soil_struct *Soil, double *tillageFactor)
{
    ...
    NFinal = 0.0;
    NInitial =
        Residue->stanResidueN + Residue->flatResidueN +
    ...
    {
        NInitial +=
            Soil->SON_Mass[i] + Soil->MBN_Mass[i] + Soil->NO3[i] +
            Soil->NH4[i] + Residue->residueAbgdN[i] + Residue->residueRtN[i] +
            Residue->residueRzN[i] + Residue->manureN[i];
        ...
        if (i == 0)
        {
            if (Residue->stanResidueMass > 0.0)
                stanCNRatio =
                    Residue->stanResidueMass * FRACTION_CARBON_PLANT /
                    Residue->stanResidueN;
            if (Residue->flatResidueMass > 0.0)
                flatCNRatio =
                    Residue->flatResidueMass * FRACTION_CARBON_PLANT /
                    Residue->flatResidueN;
            if (Residue->manureSurfaceC > 0.0)
                smanCNRatio =
                    Residue->manureSurfaceC / Residue->manureSurfaceN;
        }
        ...
        if (Residue->residueAbgd[i] > 0.0)
            abgdCNRatio =
                Residue->residueAbgd[i] * FRACTION_CARBON_PLANT /
                Residue->residueAbgdN[i];
        if (Residue->residueRt[i] > 0.0)
            rootCNRatio =
                Residue->residueRt[i] * FRACTION_CARBON_PLANT /
                Residue->residueRtN[i];
        if (Residue->residueRz[i] > 0.0)
            rhizCNRatio =
                Residue->residueRz[i] * FRACTION_CARBON_RIZHO /
        ...
        abgdHumificationFactor =
            ...
            humificationAdjustmentBySOC);
        rootHumificationFactor =
            sqrt (MaximumRootHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC);
        rhizHumificationFactor =
            sqrt (MaximumRhizHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC);
        manuHumificationFactor =
            sqrt (MaximumManuHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC);
        ...
        {
            contactFractionStan =
                pow (Residue->stanResidueTau,
                exp (-1.5 / sqrt (1.0 - Residue->stanResidueTau)));
            contactFractionFlat =
                pow (Residue->flatResidueTau,
                exp (-1.5 / sqrt (1.0 - Residue->flatResidueTau)));

            xx2 =
                SoilCarbon->factorComposite[i] *
                MAXIMUM_RESIDUE_DECOMPOSITION_RATE * contactFractionStan *
                Residue->stanResidueMass;
            xx3 =
                SoilCarbon->factorComposite[i] *
                MAXIMUM_RESIDUE_DECOMPOSITION_RATE * contactFractionFlat *
                Residue->flatResidueMass;
            xx6 =
                SoilCarbon->factorComposite[i] *
                MAXIMUM_MANURE_DECOMPOSITION_RATE * Residue->manureSurfaceC;
        }

        xx1 =
            SoilCarbon->factorComposite[i] *
            MAXIMUM_RESIDUE_DECOMPOSITION_RATE * Residue->residueAbgd[i];
        xx4 =
            SoilCarbon->factorComposite[i] * MAXIMUM_ROOT_DECOMPOSITION_RATE *
            Residue->residueRt[i];
        xx5 =
            SoilCarbon->factorComposite[i] *
            MAXIMUM_RHIZO_DECOMPOSITION_RATE * Residue->residueRz[i];
        xx7 =
            SoilCarbon->factorComposite[i] *
            MAXIMUM_MANURE_DECOMPOSITION_RATE * Residue->manureC[i];
        ...
        if (i == 0)
        {
            if (xx2 > 0.0)
            {
                ...
                nm2 =
                    NitrogenMineralization (stanCNRatio, CNnew,
                    abgdHumificationFactor, xx2 * FRACTION_CARBON_PLANT);
            }
            if (xx3 > 0.0)
            {
                CNnew = CNdestiny (NMineralConcentration, flatCNRatio);
                nm3 =
                    NitrogenMineralization (flatCNRatio, CNnew,
                    abgdHumificationFactor, xx3 * FRACTION_CARBON_PLANT);
            }
            if (xx6 > 0.0)
            {
                CNnew = CNdestiny (NMineralConcentration, smanCNRatio);
                nm6 =
                    NitrogenMineralization (smanCNRatio, CNnew,
                    manuHumificationFactor, xx6);
            }
        ...
        if (xx1 > 0.0)
        {
            ...
            nm1 =
                NitrogenMineralization (abgdCNRatio, CNnew,
                abgdHumificationFactor, xx1 * FRACTION_CARBON_PLANT);
        }
        if (xx4 > 0.0)
        {
            CNnew = CNdestiny (NMineralConcentration, rootCNRatio);
            nm4 =
                NitrogenMineralization (rootCNRatio, CNnew,
                rootHumificationFactor, xx4 * FRACTION_CARBON_PLANT);
        }
        if (xx5 > 0.0)
        {
            CNnew = CNdestiny (NMineralConcentration, rhizCNRatio);
            nm5 =
                NitrogenMineralization (rhizCNRatio, CNnew,
                rhizHumificationFactor, xx5 * FRACTION_CARBON_RIZHO);
        }
        if (xx7 > 0.0)
        {
            CNnew = CNdestiny (NMineralConcentration, manuCNRatio);
            nm7 =
                NitrogenMineralization (manuCNRatio, CNnew,
                manuHumificationFactor, xx7);
        }
        ...
        if (xxPartialSum > 0.0)
            micrHumificationFactor =
                micrHumificationFactor >
                (1.0 / xxPartialSum) * (abgdHumificationFactor * (xx1 + xx2 +
                    xx3) + rootHumificationFactor * xx4 +
                rhizHumificationFactor * xx5 + manuHumificationFactor * (xx6 +
                    xx7)) ? micrHumificationFactor : (1.0 / xxPartialSum) *
                (abgdHumificationFactor * (xx1 + xx2 + xx3) +
                rootHumificationFactor * xx4 + rhizHumificationFactor * xx5 +
        ...
        socDecompositionRate =
            ...
            decompositionAdjustmentBySOC / (1.0 - pow (micrHumificationFactor,
                2.0));
        ...
        if (xx8 > 0.0)
        {
            ...
            nm8 =
                NitrogenMineralization (somCNratio, CNnew,
         ...
         * ks/(1-e^2) */
        aux3 =
            aux2 * MAXIMUM_UNDISTURBED_SOC_DECOMPOSITION_RATE *
            decompositionAdjustmentBySOC / (1.0 - pow (micrHumificationFactor,
                2.0)) * (0.97 / 0.03) * (1.0 / micrHumificationFactor);
        micrDecompositionRate =
         ...
         * microbial pool */
        humifiedCarbon =
            abgdHumificationFactor * FRACTION_CARBON_PLANT * (xx1 + xx2 +
            xx3) + rootHumificationFactor * FRACTION_CARBON_PLANT * xx4 +
        ...
        if (i == 0)
        {
            Residue->stanResidueWater *=
                (1.0 - xx2 / (Residue->stanResidueMass + 1e-10));
            Residue->flatResidueWater *=
                (1.0 - xx3 / (Residue->flatResidueMass + 1e-10));
        ...
        SoilCarbon->carbonRespired[i] =
            (1.0 - abgdHumificationFactor) * FRACTION_CARBON_PLANT * (xx1 +
            xx2 + xx3) + (1.0 -
            rootHumificationFactor) * FRACTION_CARBON_PLANT * xx4 + (1.0 -
            rhizHumificationFactor) * FRACTION_CARBON_RIZHO * xx5 + (1.0 -
            manuHumificationFactor) * (xx6 + xx7) + (1.0 -
            micrHumificationFactor) * (xx8 + xx9);

        /* For output */
        SoilCarbon->annualSoilCarbonDecompositionRate[i] +=
            socDecompositionRate;

        /* Excludes residue and manure */
        SoilCarbon->annualRespiredCarbonMass[i] +=
            (1.0 - micrHumificationFactor) * (xx8 + xx9);

        /* Residue, roots and manure only */
        SoilCarbon->annualRespiredResidueCarbonMass[i] +=
            SoilCarbon->carbonRespired[i] - (1.0 -
            micrHumificationFactor) * (xx8 + xx9);
        SoilCarbon->abgdCarbonInput[i] +=
            FRACTION_CARBON_PLANT * (xx1 + xx2 + xx3);
        ...
        SoilCarbon->annualCarbonInputByLayer[i] =
            SoilCarbon->abgdCarbonInput[i] + SoilCarbon->rootCarbonInput[i] +
            SoilCarbon->rhizCarbonInput[i] + SoilCarbon->manuCarbonInput[i];

        /* C that goes to SOC and C that goes to microbial mass */
        SoilCarbon->annualHumifiedCarbonMass[i] +=
            xx9 * micrHumificationFactor + humifiedCarbon;
        ...
        SoilCarbon->annualNNetMineralization[i] +=
            NNetMineralization * 1000.0;
    ...
    SoilCarbon->annualAmmoniumNitrification +=
        Soil->NH4_Nitrification * 1000.0;
    SoilCarbon->annualNitrousOxidefromNitrification +=
        Soil->N2O_Nitrification * 1000.0;
    SoilCarbon->annualAmmoniaVolatilization +=
        Soil->NH4_Volatilization * 1000.0;
    SoilCarbon->annualNO3Denitrification +=
    ...
    residue_struct *Residue, soil_struct *Soil, double *tillageFactor)
{
    ...
    {
        NInitial +=
            Soil->SON_Mass[i] + Soil->NO3[i] + Soil->NH4[i] +
            Residue->residueAbgdN[i] + Residue->residueRtN[i] +
            Residue->residueRzN[i] + Residue->manureN[i];
        ...
        abgdHumificationFactor =
            MaximumAbgdHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC;
        rootHumificationFactor =
            MaximumRootHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC;
        rhizHumificationFactor =
            MaximumRhizHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC;
        manuHumificationFactor =
            MaximumManuHumificationFactor (Soil->Clay[i]) *
            humificationAdjustmentBySOC;
        ...
        if (i == 0)
        {
            if (xx2 > 0.0)
                nm2 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    stanCNRatio, abgdHumificationFactor, xx2,
                    FRACTION_CARBON_PLANT);
            if (xx3 > 0.0)
                nm3 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    flatCNRatio, abgdHumificationFactor, xx3,
                    FRACTION_CARBON_PLANT);
            if (xx6 > 0.0)
                nm6 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    smanCNRatio, manuHumificationFactor, xx6, 1.0);
        ...
        if (xx1 > 0.0)
            nm1 =
                PoolNitrogenMineralization (NO3_Concentration, abgdCNRatio,
                abgdHumificationFactor, xx1, FRACTION_CARBON_PLANT);
        if (xx4 > 0.0)
            nm4 =
                PoolNitrogenMineralization (NO3_Concentration, rootCNRatio,
                rootHumificationFactor, xx4, FRACTION_CARBON_PLANT);
        if (xx5 > 0.0)
            nm5 =
                PoolNitrogenMineralization (NO3_Concentration, rhizCNRatio,
                rhizHumificationFactor, xx5, FRACTION_CARBON_RIZHO);
        if (xx7 > 0.0)
            nm7 =
                PoolNitrogenMineralization (NO3_Concentration, manuCNRatio,
                manuHumificationFactor, xx7, 1.0);
        ...
        if (decompReductionFactor < 1.0)
        {
            ...
            if (nm1 < 0.0)
                nm1 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    abgdCNRatio, abgdHumificationFactor, xx1,
                    FRACTION_CARBON_PLANT);
            if (nm2 < 0.0)
                nm2 =
                    PoolNitrogenMineralization (NO3_Concentration,
            ...
            if (nm4 < 0.0)
                nm4 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    rootCNRatio, rootHumificationFactor, xx4,
                    FRACTION_CARBON_PLANT);
            if (nm5 < 0.0)
                nm5 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    rhizCNRatio, rhizHumificationFactor, xx5,
                    FRACTION_CARBON_RIZHO);
            if (nm6 < 0.0)
                nm6 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    smanCNRatio, manuHumificationFactor, xx6, 1.0);
            if (nm7 < 0.0)
                nm7 =
                    PoolNitrogenMineralization (NO3_Concentration,
                    manuCNRatio, manuHumificationFactor, xx7, 1.0);
        }
        ...
        apparentSOCDecompositionRate =
            ...
            decompositionAdjustmentBySOC;
        apparentSOCDecomposition =
            apparentSOCDecompositionRate * Soil->SOC_Mass[i];
        ...
        Soil->SON_Mass[i] +=
            humifiedNitrogen + (-NImmobilization) - SON_Mineralization;
        Soil->SOC_Conc[i] = Soil->SOC_Mass[i] * 1000.0 / soilMass;
        SoilCarbon->carbonRespired[i] =
            apparentSOCDecomposition + (1.0 -
            abgdHumificationFactor) * FRACTION_CARBON_PLANT * (xx1 + xx2 +
            xx3) + (1.0 -
            rootHumificationFactor) * FRACTION_CARBON_PLANT * xx4 + (1.0 -
            rhizHumificationFactor) * FRACTION_CARBON_RIZHO * xx5 + (1.0 -
        ...
        SoilCarbon->annualRespiredResidueCarbonMass[i] +=
            SoilCarbon->carbonRespired[i] - apparentSOCDecomposition;
        SoilCarbon->abgdCarbonInput[i] +=
            FRACTION_CARBON_PLANT * (xx1 + xx2 + xx3);
        SoilCarbon->rootCarbonInput[i] += FRACTION_CARBON_PLANT * xx4;
        SoilCarbon->rhizCarbonInput[i] += FRACTION_CARBON_RIZHO * xx5;
...
double TemperatureFunction (double T)
{
    ...
    else
    {
        ...
        temp =
            (pow (T - tMin, Q) * (tMax - T)) / (pow (tOpt - tMin,
                Q) * (tMax - tOpt));
        if (temp > 1.0)
            temp = 1.0;
    }
...
double NitrogenMineralization (double CNDecomposing, double CNnew,
    double humRate, double decomposedMass)
{
...
double CNdestiny (double NmineralConc, double CNdecomposing)
{
    ...
    return (8.5 + YY2 * (0.5 + 0.5 / (1.0 + pow (NmineralConc / 0.000008,
                    3.0))));
}

double PoolNitrogenMineralization (double NmineralConc,
