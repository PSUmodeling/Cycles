...
{
    double          PMET;
    double          rc = 0.00081;   /* surface resistance to vapor flow, day/m
                                     * (0.00081 for 350-400 ppm) */
    double          CP = 0.001013;  /* specific heat of dry air, J/(kg C) */
    double          R = 0.28704;    /* specific gas constant for dry air
                                     * kJ/(kg degree C)
    ...
    delta = 4098. * esTave / (pow (Tave + 237.3, 2));   
    ...
    aeroTerm = (volCP * vpd / ra) / (delta + gamma * (1. + rc / ra));

    /* radiation term, MJ/m2 */ 
    radTerm = delta * netRad / (delta + gamma * (1. + rc / ra));    
    ...
    if (PMET < 0)
        ...
        return PMET;            /* preventing a negative value usually small
                                 * and indicative of condensation */
}
...
double NetRadiation (double Pot_Rad, double Solar_Rad, double Actual_VP, double TMax, double TMin)
{
    ...
    LWR = 86400. * 0.0000000567 * 0.5 * (pow (TMax + 273.15, 4) + pow (TMin + 273.15, 4)) / 1000000.;
